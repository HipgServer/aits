...
export default function LogIncidentPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in duration-300">
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-xs text-sky-100 mt-0.5 font-medium">
              Submit workplace accident, incident, injury or near-miss event logs into database.
            </p>
          </div>
        </div>
      </div>
      ...
      {/* Form Container */}
      <form onSubmit={handleSubmit} className="glass-card p-6 md:p-8 rounded-3xl border border-slate-200 space-y-6 shadow-sm bg-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
          ...
          <div>
            <label className="block text-slate-800 font-bold mb-1.5">
              Date & Time of Event <span className="text-rose-500">*</span>
            </label>
            <input
              type="datetime-local"
              value={dateTime}
        ...
        {selectedEmp && (
          <div className="p-3.5 rounded-2xl bg-sky-50 border border-sky-200 flex items-center gap-3 text-xs">
            <UserCheck className="w-5 h-5 text-sky-600 shrink-0" />
            <div className="text-slate-800">
              <span className="font-bold text-slate-900">{selectedEmp.name}</span> • ID Card: <strong>{selectedEmp.idnum}</strong> • Employer: <strong>{selectedEmp.employer}</strong> • Role: {selectedEmp.role} ({selectedEmp.dept})
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
          ...
          <div>
            <label className="block text-slate-800 font-bold mb-1.5">
              Shift Pattern <span className="text-rose-500">*</span>
            </label>
            <select
              value={shift}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
          ...
          <div>
            <label className="block text-slate-800 font-bold mb-1.5">
              Equipment Details (Machinery involved) <span className="text-rose-500">*</span>
            </label>
            <input
              type="text"
        ...
        <div className="text-xs">
          <label className="block text-slate-800 font-bold mb-1.5">
            Reported By (Supervisor / Inspector Name)
          </label>
          <input
            type="text"
        ...
        <div className="space-y-3 text-xs pt-2">
          <label className="block text-slate-800 font-bold">
            Attach Supporting PDF Documents <span className="text-slate-400 font-normal">(optional, max 10 MB each)</span>
          </label>
        ...
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-200">
          <button
            type="button"
            onClick={handleClear}
            className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-slate-100 text-slate-700 font-bold hover:bg-slate-200 transition-colors text-xs"
          >
